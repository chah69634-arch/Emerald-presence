...
7.  yexuan / hongcha mid_term buckets are isolated
8.  hongcha format_for_prompt does not contain yexuan unique word
...
def test_load_reads_from_resolver_path(sandbox):
    ...
    events = _mt.load(_UID, char_id="hongcha")
...
def test_append_writes_to_resolver_path(sandbox):
    ...
    _mt.append(_UID, "append-sentinel", tags=["t1"], char_id="hongcha")
...
def test_format_for_prompt_reads_from_resolver_path(sandbox):
    ...
    result = _mt.format_for_prompt(_UID, char_id="hongcha")
...
def test_mid_term_path_equals_legacy_sandbox_path(sandbox):
    ...
    legacy_path = sandbox.user_memory_root(_UID, char_id="hongcha") / "mid_term.json"
...
def test_yexuan_hongcha_mid_term_isolated(sandbox):
    import core.memory.mid_term as _mt

    _mt.append(_UID, "叶瑄专属摘要", char_id="yexuan")
    _mt.append(_UID, "红茶专属摘要", char_id="hongcha")

    y_events = _mt.load(_UID, char_id="yexuan")
    h_events = _mt.load(_UID, char_id="hongcha")

    y_summaries = [e["summary"] for e in y_events]
    h_summaries = [e["summary"] for e in h_events]

    assert "叶瑄专属摘要" in y_summaries
    assert "红茶专属摘要" not in y_summaries
    assert "红茶专属摘要" in h_summaries
    assert "叶瑄专属摘要" not in h_summaries

    y_path = sandbox.user_memory_root(_UID, char_id="yexuan") / "mid_term.json"
    h_path = sandbox.user_memory_root(_UID, char_id="hongcha") / "mid_term.json"
...
def test_format_for_prompt_isolation(sandbox):
    ...
    hongcha_word = "hongcha-unique-9b2e1"

    _mt.append(_UID, yexuan_word, char_id="yexuan")
    _mt.append(_UID, hongcha_word, char_id="hongcha")

    y_text = _mt.format_for_prompt(_UID, char_id="yexuan")
    h_text = _mt.format_for_prompt(_UID, char_id="hongcha")

    assert yexuan_word in y_text, f"yexuan word missing from yexuan format_for_prompt"
    assert yexuan_word not in h_text, f"yexuan word leaked into hongcha format_for_prompt"
    assert hongcha_word in h_text, f"hongcha word missing from hongcha format_for_prompt"
    assert hongcha_word not in y_text, f"hongcha word leaked into yexuan format_for_prompt"
...
def test_mark_promoted_writes_to_resolver_path(sandbox):
    ...
    _mt.append(_UID, "promoted-test", mid_id="mid-promo-1", char_id="hongcha")
    assert expected_path.exists()

    _mt.mark_promoted(_UID, "mid-promo-1", "ep-001", char_id="hongcha")
